...
class ScalableImage extends StatefulWidget {
  ...
  const ScalableImage({Key key,@required ImageProvider imageProvider,double maxScale,double dragSpeed,Size size,bool wrapInAspect,bool enableScaling,Size screenSize}):
  ...
  Widget build(BuildContext context) {
    if(_imageInfo==null){
      ...
      Widget painter=new CustomPaint(
        size: widget._size,
        painter: new _ScalableImagePainter(_imageInfo.image,_offset,_scale,widget._screenSize),
        willChange: true,
      );
...
class _ScalableImagePainter extends CustomPainter{
  ...
  _ScalableImagePainter(this._image,Offset offset,double scale,Size screenSize) :
        this.scalevalue=scale??1.0,
  ...
  void paint(Canvas canvas, Size size) {
    double scrwid = _screenSize.width;
    double srchei = _screenSize.height;
    double dleft = 0.0;
    double dtop = 0.0;
    double dwidth = size.width*scalevalue;
    double dheight = size.height*scalevalue;

    dtop = (1.0-scalevalue)*dheight/2;
    dheight-=dtop;
    dleft = (1.0-scalevalue)*dwidth/2;
    dwidth-=dleft;

    canvas.drawImageRect(
        _image,
        _rect,
        new Rect.fromLTWH(dleft, dtop, dwidth, dheight),
        _paint);
  }
